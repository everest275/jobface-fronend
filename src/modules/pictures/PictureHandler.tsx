...
import { PortfolioPicture } from './PictureService';
...
const Base64ImageHandler: React.FC<PictureHandlerProps> = ({ id, isViewer, isPublic,type }) => {
  ...
  const { updatePortfolioPicture, getPortfolioPictureByPortfolioId, changePortfolioPicture, getPortfolioPictureByPortfolioIdPublic } = usePortfolioPictures()
  ...
  useEffect(() => {
    if (id) {
      const loadPicture = async () => {
        ...
        if(type===1){
          if (isPublic) {
            const picture = await getPortfolioPictureByPortfolioIdPublic(id);
            if (picture) {
              setPortfolioPicture(picture)
              setBase64Image(picture.picture_data)
            }
          }
  
          const picture = await getPortfolioPictureByPortfolioId(id);
          if (picture) {
            setPortfolioPicture(picture)
            setBase64Image(picture.picture_data)
          }  
  ...
  }, [id,type, isPublic, getPortfolioPictureByPortfolioIdPublic, getPortfolioPictureByPortfolioId, setBase64Image]);
  ...
  const handleSavePicture = async () => {
    ...
    if(type===1){
      ...
      if (portfolioPicture) {
        ...
        await changePortfolioPicture(portfolioPicture.id, pullRequest)
        portfolioPicture.picture_data = pullRequest.picture_data
        setPortfolioPicture(portfolioPicture)
  ...
  return (
    <div className="flex flex-col gap-6 ">
      ...
      <div className="self-center flex justify-center items-center w-24 h-24 md:w-56 md:h-56 rounded-[100%] shadow-2xl bg-gray-200 hover:cursor-pointer hover:scale-110 duration-300  bg-gradient-to-r from-teal-500 via-purple-500 to-orange-500">
        <img className={portfolioPicture ? "w-full h-full rounded-[100%] border-4 border-transparent" : "w-14 h-14"} src={portfolioPicture ? portfolioPicture.picture_data : editPhotoIcon} alt="Encoded" />
      </div>

      {isViewer && (
        <button
          onClick={handleButtonClick}
          className="py-2 px-4 mt-4 self-center bg-blue-500 text-white rounded hover:bg-blue-700"
        >
          Cambiar foto
        </button>
      )}
